...
// Version : 2.0
// =============================================
// 1. Bug critique : blur déclenché lors de l'affichage de la modal
//    → ajout d'un garde "modalOuverte" pour éviter les double-warnings
// 2. Bug critique : MAX_AVERTISSEMENTS = 2 trop bas, sans soumission forcée
//    → MAX_AVERTISSEMENTS = 3, soumission automatique déclenchée
// 3. Détections absentes : copy/paste levaient preventDefault sans warning
//    → ajout d'un avertissement pour chaque action
// 4. Raccourcis clavier incomplets (Ctrl+Shift+I sensible à la casse)
//    → utilisation de e.key.toLowerCase() et ajout de Ctrl+Shift+J, Ctrl+S
// 5. Pas de détection de focus perdu sur window (différent de blur document)
//    → confirmé et conservé sur window, avec garde modal
// 6. fullscreenchange non préfixé pour Safari/Firefox
//    → ajout des variantes webkit et moz
// =============================================
...
/**
 * Nombre total de questions dans le DOM.
 * Calculé une seule fois au chargement (évite un querySelector à chaque clic).
 */
const nbQuestions = document.querySelectorAll('.question').length;

/**
 * Change la question affichée.
 *
 * @param {number} direction - +1 pour avancer, -1 pour reculer.
 *
 * Fonctionnement :
 * 1. Retire la classe "active" de la question courante (la cache via CSS).
 * 2. Calcule le nouvel index, clampé entre 0 et nbQuestions - 1.
 * 3. Ajoute "active" à la nouvelle question (l'affiche).
 * 4. Met à jour le compteur affiché et l'état des boutons.
 */
function changerQuestion(direction) {
    ...
    // Mise à jour du compteur "Question X/10"
    ...
    // Bouton "Suivant" masqué sur la dernière question, "Valider" visible
    document.getElementById('btn-suivant').classList.toggle(
        'hidden', questionActuelle === nbQuestions - 1
    );
    document.getElementById('btn-valider').classList.toggle(
        'hidden', questionActuelle !== nbQuestions - 1
    );
}
